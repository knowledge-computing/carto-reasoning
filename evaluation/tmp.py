...
def winsorized_bootstrap(data, proportion=0.25, n_boot=1000):
    means = []
    # data = data * 4
    for _ in range(n_boot):
        # sample = random.choice(data)
        sample = np.random.choice(data, size=len(data), replace=True)
        wins = mstats.winsorize(sample, limits=proportion)
        # means.append(statistics.mean(wins))
        means.append(np.mean(wins))
    return means

# winsorized_means = winsorized_bootstrap(data)
# print("Winsorized Bootstrap SD:", statistics.stdev(winsorized_means))
...
def std_calculation(expected_answer, annotator_response):
    ...
    winsorized_means = statistics.stdev(winsorized_bootstrap(list_annotator))
    
    return {'annotator_response': list_annotator, 
            'mean': mean_value,
            'median': median_value,
            'std': std_value,
            'win_std': winsorized_means,
